...
| 要素ID | 要素チャット名 | フェーズ | 主担当 | 依存 | 内容概要 | 状況 | 期日 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **SYS_CORE** | 共通基盤構築 | Phase1 | 開発管理者 | - | shared/config.php・DB・env・AIコア雛形 | ⏳ | 11/15 |
| **API_COMM** | 通信API層 | Phase2 | 開発管理者 | SYS_CORE | ChatWork／Twilio／LINE／OpenAI／STORES／RemoteLock API疎通確認 | ⏳ | 11/20 |
| **AI_CORE** | AIコア設計 | Phase1〜2 | 本部AIエンジニア | SYS_CORE | ai_core.php／意図分類／生成／チューニングUI／CSV管理 | 🟡 | 11/20 |
| **HELP_UI** | 顧客チュートリアル（Web） | Phase3 | フロント開発者 | AI_CORE／API_COMM | help./tutorial_api／AI連携／FAQ表示 | ⏳ | 11/30 |
| **LINE_UI** | 顧客チュートリアル（LINE） | Phase3 | LINE開発者 | AI_CORE／API_COMM | webhook.php／Messaging API／LIFF／ログイン／本人認証 | ⏳ | 11/30 |
| **SHOP_PORTAL** | 加盟店ポータル | Phase4 | 開発管理者／サポートチーム | HELP_UI | 顧客対応／請求／申請／備品／AI相談／MTG予約 | ⏳ | 12/07 |
| **AI_INBOX** | 本部AI-INBOX | Phase5 | 本部管理者 | SHOP_PORTAL／AI_CORE | 全チケット統合／承認／FAQ／AI設定UI | ⏳ | 12/14 |
| **HQ_CHAT** | 本部AIチャット／AI相談 | Phase4 | サポートチーム | SHOP_PORTAL／AI_CORE | hq_chat_threads／AI一次回答→本部対応 | ⏳ | 12/07 |
| **HQ_MEETINGS** | 本部MTG予約 | Phase4 | 管理チーム | SHOP_PORTAL | hq_meeting_slots／hq_meetings／リマインド通知 | ⏳ | 12/07 |
| **SALES_LINK** | STORES売上取込・KPI | Phase6 | 分析担当 | AI_INBOX | sales_records／shop_kpis／分析UI | ⏳ | 12/20 |
| **LOCK_API** | RemoteLock連携 | Phase2〜6 | 外部連携担当 | API_COMM／AI_CORE | 解錠PIN照合／STORES予約照合／本人確認 | ⏳ | 12/20 |
| **OP_MAIL** | 電話代行メール解析 | Phase7 | 運用保守チーム | AI_CORE | IMAP取込→AI分類→要対応チケット化→ChatWork＋SMS | ⏳ | 12/25 |
| **CRON_SYS** | cron／自動処理 | Phase7 | 運用保守チーム | SYS_CORE／AI_INBOX | 請求／SMS追撃／バックアップ／リマインド | ⏳ | 12/25 |
| **KPI_ANALYTICS** | KPI分析 | Phase6 | 分析担当 | SALES_LINK／AI_CORE | AI応答率／顧客満足／売上分析 | ⏳ | 12/20 |
| **FAQ_MANAGE** | FAQ管理UI | Phase5 | 本部管理者 | AI_CORE | intents CRUD／CSV編集／FAQ更新 | ⏳ | 12/14 |
| **AI_TUNE** | AIチューニング | Phase8 | 本部AIエンジニア | AI_CORE／FAQ_MANAGE | パラメータ学習・モデル最適化 | 🔄 | 継続 |
| **PACKAGE_BUILD** | パッケージ生成 | Phase9 | 開発管理者 | 全要素 | ZIP構成出力・selfcheck・smoke実行 | ⏳ | 12/30 |
