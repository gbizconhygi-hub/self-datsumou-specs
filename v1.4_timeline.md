...
| **Phase 2：通信・外部連携層** | 通知・API通信 | ChatWork／Twilio／LINE Messaging & Login／OpenAI／STORES API／RemoteLock API（インターフェースのみ） | ②⑨ |
...
| **Phase 6：データ連携／KPI層** | 収益・AI成果分析 | sales_records の取込バッチ／tutorial_logs／shop_kpis／ダッシュボード | ⑥ |
| **Phase 7：自動処理層（バックグラウンド）** | 自動化・監視 | 請求自動化／SMS追撃／オペレーター報告解析／バックアップ／監査 | ⑦⑨ |
...
| **API_COMM** | 通信API層 | Phase2 | 開発管理者 | SYS_CORE | ChatWork／Twilio／LINE／OpenAI／STORESAPI疎通確認／RemoteLOCKインターフェース（スタブ） |
...
| **SALES_LINK** | STORES売上取込・KPI | Phase6 | 分析担当 | AI_INBOX | sales_records の取込バッチ／shop_kpis／分析UI |
| **LOCK_API** | RemoteLock連携 | Phase9 | 外部連携担当 | API_COMM／AI_CORE | 解錠PIN照合／STORES予約照合／本人確認（将来） |
| **OP_MAIL** | 電話代行メール解析 | Phase7 | 運用保守チーム | AI_CORE | 集約メールボックス新着→AI分類→要対応チケット化→ChatWork＋SMS |
...
| --- | --- | --- | --- | --- | --- |
| Phase 0 | 1 | システム管理者 | — | 完了済 | 環境動作基盤（SSL/DB/mail）＋ Phase0 基本仕様（FOUNDATION） |
| Phase 1 | 2 | 開発管理者 | 0 | 完了済 | コア構造・DB・.env固定・AI雛形 |
| Phase 2 | 3 | 開発管理者／外部連携担当 | 1 | 完了済 | 通信API・外部サービス連携確認 |
| Phase 3 | 4 | AI運用チーム | 2 | 完了済 | help.／line. 顧客AIチュートリアル完成 |
| Phase 4 | 5 | 加盟店サポートチーム | 3 | 進行中 | shop.ポータル／AI相談／MTG予約 |
| Phase 5 | 6 | 本部管理者 | 4 | 未着手 | ai.INBOX／FAQ管理／AI設定UI |
| Phase 6 | 7 | 分析担当 | 5 | 未着手 | KPIダッシュボード・sales連携 |
| Phase 7 | 8 | 運用保守チーム | 5 | 未着手 | cron・バックアップ・監査／自動通知 |
