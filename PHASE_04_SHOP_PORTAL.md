...
- SHOP_PORTAL の UI デザイン（カラー／雰囲気）は、  
  **ハイジ公式ページ（https://self-datsumou.com）とブランド整合を取ることを前提とする**。
- 方針レベルでの決定事項：
...
- OP_MAIL（電話代行メール解析／エスカレーション）は Phase7 で実装され、  
  `tickets.source='operator_mail'` のチケットを生成する。  
...
2. 顧客問い合わせ（要対応分）
   ...
   - `source='operator_mail' AND severity='urgent'` のチケットは「電話代行（緊急）」ラベルを付け、最上段に固定表示
3. 店舗契約・オプション情報（閲覧のみ）
...
2. 「顧客問い合わせ」タブ：
   ...
   - `source='operator_mail' AND severity='urgent'` を最上段に固定表示し、「電話代行（緊急）」バッジを表示
